const y = new Date().getFullYear()

export const projectDetailsData = {
    content: `<h5>بررسی پروژه</h5><p>گیاهان گاز می‌گیرند تا پرندگان را از مزاحمت برای ذرت بازدارند و مثل یک گربه نینجا عمل کنند و از این کار لذت ببرند، همه جا حضور دارند و مزاحمت می‌کنند، تمام روز روی جعبه خاک خود رنگین‌کمان درست می‌کنند، روی سر انسان‌ها می‌خوابند و از سگ‌ها نفرت دارند، به موش حمله می‌کنند، نخ می‌خورند و روی بالش بالا می‌آورند. عاشق حمام نیستند، و همه چیز را دوست ندارند. در اطراف خانه مدفوع می‌کنند، سگ را دنبال می‌کنند و فرار می‌کنند، اما وقتی نوازش‌شان کنی، نه آنجا، اینجا باید نوازش کنی،</p><h5>درباره‌ی مشتری</h5><p>لیسیدن خود. زیر تخت یا با خرخر یا حمله مثل یک هیولا و سپس ناگهان شروع به دویدن سریع در سراسر خانه بدون هیچ دلیل خاصی می‌کنند. در ساعت ۴ صبح جیغ می‌زنند یا به پا می‌چسبند و میومیو می‌کنند. گربه روی درخت گیر می‌کند، آتش‌نشان‌ها سعی می‌کنند گربه را پایین بیاورند، اما آتش‌نشان‌ها هم گیر می‌کنند، گربه دمپایی‌های آتش‌نشان‌ها را می‌خورد، در خانه می‌دود، از پله‌ها بالا و پایین می‌رود تا اشباح را دنبال کند، روی گوشی شما می‌خوابد و خرخرهای بامزه می‌کند، جوراب را بو می‌کند و با دهان نیمه باز راه می‌رود، در جعبه می‌نشیند، دیوارها را بو می‌کند، انگار صبحانه خورده است. گلوله مویی بالا می‌آورد، گربه کوچک و بامزه.</p><ul><li>چی شد؟ چیزی مشکوک به نظر می‌رسد. سرش را به ما می‌مالد و مدام گریه می‌کند، مگر اینکه نوازشش کنیم، و شاید هم برای سرگرمی گریه کند. تقاضا می‌کند که از هر چه انسان می‌پزد، به او بدهیم، سپس آن را بو می‌کند و دور می‌شود، پرده را لیس می‌زند فقط برای اذیت کردن، در باز می‌شود! چقدر هیجان‌انگیز! اوه، تویی، بی‌اهمیت. از روی میز می‌پرد، ماهی می‌خورد، بشقاب را از روی میز پرت می‌کند، سرش را می‌کوبد، نمی‌تواند از بشقاب خودش بخورد، به دمم دست بزنی، دستت را پاره می‌کنم. میومیو.</li><li>به سمت شخصی غافل حمله می‌کند، بلندترین صدای خرخر را در می‌آورد، همه چیز را از روی میز پرت می‌کند، اما پنجه‌هایش را روی شکم انسان کشیده و مانند ماشین چمن‌زنی خرخر می‌کند و درخواست نوازش می‌کند. گربه‌ها موجودات خاصی هستند، بنابراین نخ‌های آویزان را می‌لیسند، روی زمین می‌لغزند، به دیوار برخورد می‌کنند.</li><li>میومیو، تو صاحب منی، پس این موش مرده رو بگیر. از ظاهر ناگهانی خیار ترسید و حیوانات مرده را به عنوان هدیه گذاشت. گربه روی کاناپه می‌پرد، خرخر می‌کند و در دنیای جدیدی پر از خاک گربه بیدار می‌شود. میومیو، خوشمزه، کلی گربه دیگر اینجا هستند و در حال خوردن کت‌نیپ هستند.</li></ul><img src="/static/img/others/article-img-1.jpg" alt="" /><h5>اهداف</h5><p>پاداش به انسان انتخاب شده با یک پلک آرام، به طوری که مقداری کرک به چشم صاحب برسد و روی انسان بنشیند. بوی علف می‌دهد و سپس گربه‌ها دنیا را تسخیر می‌کنند، اما به چراغ سقف خیره می‌شوند، دماغ گربه دیگر را بو می‌کنند و دهان‌شان نیمه باز می‌ماند. ۴ ساعت نظافت - تیک، ۱۸ ساعت خواب زیبایی - تیک، بقیه روز بی‌نقص - تیک، همه جا مو می‌ریزند، پاهایت را گاز می‌گیرند، نقطه قرمز را دنبال می‌کنند، گلوله مویی، کت‌نیپ، علف می‌خورند، با سرعت تمام بالا و پایین می‌روند. گربه‌ها موجودات عجیبی هستند.</p><h5>نیازمندی‌ها</h5><p>شکم نرم و پنجه‌های کوچک، نه، نمی‌تونی در رو ببندی، هنوز تصمیم نگرفتم بیرون برم یا نه، پشت کاناپه قایم می‌شم تا با اسباب‌بازی پرزدارم بازی کنم. لیزر قرمز را دنبال می‌کنم. گوش‌های خودم را می‌خورم، ماهی روی زمین می‌خورم، گلوله مویی در کاسه غذایم می‌اندازم و از صاحبم غذای جدید طلب می‌کنم، صبح‌ها صورتم رو با پنجه‌هایم نوازش می‌کنم تا بیدار شوی.</p>`,
    activities: [],
    members: [],
    tasks: [],
    client: {
        clientName: 'Acme Agency inc',
        skateHolder: {
            name: 'Jeremiah Minsk',
            img: '/static/img/avatars/thumb-2.jpg',
        },
        projectManager: {
            name: 'Max Alexander',
            img: '/static/img/avatars/thumb-3.jpg',
        },
    },
    schedule: {
        startDate: 1709994804,
        dueDate: 1741502004,
        status: 'در حال انجام است',
        completion: 80,
    },
}

export const projectListData = [
    {
        id: '27',
        name: 'سرویس تکاملی SaaS',
        category: 'برنامه وب',
        desc: 'بیشتر شما با فضایل یک برنامه‌نویس آشنا هستید',
        attachmentCount: 12,
        totalTask: 32,
        completedTask: 27,
        progression: 80,
        dayleft: 21,
        favourite: true,
        member: [
            {
                name: 'روبرتا هورتون',
                img: '/static/img/avatars/thumb-8.jpg',
            },
            {
                name: 'یوجین استوارت',
                img: '/static/img/avatars/thumb-5.jpg',
            },
        ],
    },
    {
        id: '28',
        name: 'AIA Bill App',
        category: 'برنامه موبایل',
        desc: 'ما دستگاه شما را ارسال نمی‌کنیم!',
        attachmentCount: 5,
        totalTask: 36,
        completedTask: 15,
        progression: 45,
        dayleft: 19,
        favourite: true,
        member: [
            {
                name: 'Angelina Gotelli',
                img: '/static/img/avatars/thumb-1.jpg',
            },
        ],
    },
    {
        id: '29',
        name: 'IOP Web',
        category: 'برنامه وب بک‌اند',
        desc: 'دو راه برای نوشتن برنامه‌های بدون خطا وجود دارد؛ فقط راه سوم کار می‌کند.',
        attachmentCount: 8,
        totalTask: 27,
        completedTask: 19,
        progression: 73,
        dayleft: 6,
        favourite: false,
        member: [
            {
                name: 'مکس الکساندر',
                img: '/static/img/avatars/thumb-3.jpg',
            },
            {
                name: 'کامیلا سیمونز',
                img: '/static/img/avatars/thumb-9.jpg',
            },
        ],
    },
    {
        id: '31',
        name: 'Octonine POS',
        category: 'برنامه بک‌اند',
        desc: 'هر چیزی که می‌توان اختراع کرد، اختراع شده است.',
        attachmentCount: 8,
        totalTask: 78,
        completedTask: 23,
        progression: 21,
        dayleft: 52,
        favourite: true,
        member: [
            {
                name: 'ارل مایلز',
                img: '/static/img/avatars/thumb-10.jpg',
            },
            {
                name: 'روبرتا هورتون',
                img: '/static/img/avatars/thumb-8.jpg',
            },
            {
                name: 'آرلین پیرس',
                img: '/static/img/avatars/thumb-6.jpg',
            },
            {
                name: 'استیو ساتون',
                img: '/static/img/avatars/thumb-11.jpg',
            },
            {
                name: 'آلوین مورنو',
                img: '/static/img/avatars/thumb-14.jpg',
            },
        ],
    },
    {
        id: '30',
        name: 'Evo SaaS API',
        category: 'خدمات بک‌اند',
        desc: 'اشکال‌زدایی دو برابر سخت‌تر از نوشتن کد در وهله اول است.',
        attachmentCount: 2,
        totalTask: 15,
        completedTask: 13,
        progression: 87,
        dayleft: 2,
        favourite: true,
        member: [
            {
                name: 'استیو ساتون',
                img: '/static/img/avatars/thumb-11.jpg',
            },
        ],
    },
    {
        id: '32',
        name: 'Posiflex Web',
        category: 'برنامه وب فرانت‌اند',
        desc: 'وظیفه نرم‌افزار خوب این است که پیچیدگی را ساده جلوه دهد.',
        attachmentCount: 6,
        totalTask: 18,
        completedTask: 9,
        progression: 50,
        dayleft: 6,
        favourite: false,
        member: [
            {
                name: 'میریام هررا',
                img: '/static/img/avatars/thumb-12.jpg',
            },
        ],
    },
    {
        id: '33',
        name: 'FoksMart APP',
        category: 'برنامه موبایل',
        desc: 'موضوع بیت‌ها، بایت‌ها و پروتکل‌ها نیست، بلکه سود، زیان و حاشیه‌ها است.',
        attachmentCount: 3,
        totalTask: 26,
        completedTask: 19,
        progression: 67,
        dayleft: 14,
        favourite: false,
        member: [
            {
                name: 'شانون بیکر',
                img: '/static/img/avatars/thumb-4.jpg',
            },
            {
                name: 'جسیکا ولز',
                img: '/static/img/avatars/thumb-8.jpg',
            },
        ],
    },
    {
        id: '34',
        name: 'FlowBuzz SEO',
        category: 'بازاریابی',
        desc: 'مبل را نابود کن، کیسه پلاستیکی را لیس بزن و به صاحبش با نگاه متهمانه خیره شو و سپس چشمک بزن.',
        attachmentCount: 3,
        totalTask: 26,
        completedTask: 19,
        progression: 88,
        dayleft: 18,
        favourite: false,
        member: [
            {
                name: 'Angelina Gotelli',
                img: '/static/img/avatars/thumb-1.jpg',
            },
            {
                name: 'روبرتا هورتون',
                img: '/static/img/avatars/thumb-7.jpg',
            },
        ],
    },
    {
        id: '35',
        name: 'Mind Blast APP',
        category: 'برنامه موبایل',
        desc: 'عاشق چیزبرگرها است و ناگهان به حالت دیوانه‌وار و وحشیانه‌ای می‌رود و از توالت می‌نوشد.',
        attachmentCount: 3,
        totalTask: 74,
        completedTask: 31,
        progression: 42,
        dayleft: 37,
        favourite: false,
        member: [
            {
                name: 'کاساندرا موری',
                img: '/static/img/avatars/thumb-13.jpg',
            },
            {
                name: 'جکی سوتو',
                img: '/static/img/avatars/thumb-15.jpg',
            },
        ],
    },
]

const toBeProcessCards = [
    {
        id: 'zb7zxtjctd',
        name: 'عدم توانایی در بارگذاری فایل',
        description:
            'مهمانی هنری پایدار هشتگ سنستر ۳ گرگ ماه لبه زنده پورتلند افال پاکسازی استاد مختل ویروسی تاکوهای پالو سانتو. افال سیرچا شما احتمالاً از آنها نشنیده‌اید ویرچوال گلاسیه.',
        cover: '/static/img/others/img-13.jpg',
        members: [
            {
                id: '1',
                name: 'Angelina Gotelli',
                email: 'eileen_h@hotmail.com',
                img: '/static/img/avatars/thumb-1.jpg',
            },
            {
                id: '2',
                name: 'جرمایا مینسک',
                email: '',
                img: '/static/img/avatars/thumb-2.jpg',
            },
            {
                id: '3',
                name: 'مکس الکساندر',
                email: 'ronnie_vergas@infotech.io',
                img: '/static/img/avatars/thumb-3.jpg',
            },
        ],
        labels: ['وظیفه', 'مسئله زنده'],
        attachments: [
            {
                id: 'jubuK7XGp3',
                name: 'mail.jpg',
                src: '/static/img/others/img-13.jpg',
                size: '36.1kb',
            },
            {
                id: 'xsb3HCejCM',
                name: 'mail.jpg',
                src: '/static/img/others/img-14.jpg',
                size: '55.9kb',
            },
        ],
        comments: [
            {
                id: 'R22TqMkACm',
                name: 'کامیلا سیمونز',
                src: '/static/img/avatars/thumb-9.jpg',
                message:
                    'من بچه کیچ پشمی سبیل، قصاب ویلیامزبرگ بدون گلوتن ۳ گرگ ماه کینوا اصیل سلوِیج کناوسگارد یونیکورن هستم. پالو سانتو ویروسی حمل روزانه، تامبلر ارثی جین خام سال ایسلند ویفرهای قهوه تک‌منشأ کیف دستی شوریدچ نان ابری پوک.',
                date: new Date(y, 4, 20),
            },
        ],
        dueDate: new Date(y, 7, 5),
        checked: false,
    },
    {
        id: '7qgsduurxt',
        name: 'داده‌های جدول نادرست',
        description:
            'اضافه برِو فرنچ پرس شیر ارگانیک لانگو بار. در، کاراجیلو، نعلبکی لاته آمریکانو و فروشگاه فرنچ پرس غنی. و باریستا مازاگران، سیاه، فنجان استخراج ریسترتو گلدان پیستون دارچین کافه او لایت.',
        cover: '',
        members: [
            {
                id: '8',
                name: 'جسیکا ولز',
                email: 'iamfred@imaze.infotech.io',
                img: '/static/img/avatars/thumb-8.jpg',
            },
        ],
        labels: ['اشکال'],
        attachments: [],
        comments: [
            {
                id: 'Wx8FDSsVTg',
                name: 'آرلین پیرس',
                src: '/static/img/avatars/thumb-6.jpg',
                message:
                    'هلفتیکا ۸-بیت فوتوبوث تامبلر کامیون غذا. پین مینای دندان گرگ شلوغ سارتوریال، برانچ شوریدچ اسکیت‌بورد ریش هلفتیکا. تایپ‌نویس گاستروپاب سفارشی.',
                date: new Date(y, 4, 20),
            },
            {
                id: '3AhkqqSFFr',
                name: 'مکس الکساندر',
                src: '/static/img/avatars/thumb-3.jpg',
                message:
                    'اف تراست فاند مگینگز، مدیتیشن چیچارونز برانچ گرگ ناروال شوریدچ هرچه حقوق دوچرخه رمپ‌ها.',
                date: new Date(y, 4, 20),
            },
        ],
        dueDate: new Date(y, 6, 11),
    },
    {
        id: 'wtwgpz6csc',
        name: 'رفع رابط کاربری خراب',
        description:
            'گیاه هوا کاشی مترو چهار لوکو رمپ‌ها. میکرو دوزینگ افال کیف دستی، قهوه تک‌منشأ بیودیزل قبل از اینکه آنها را بفروشند سوگ پوک پوک گاستروپاب تاکوها لترپرس.',
        cover: '',
        members: [
            {
                id: '13',
                name: 'کاساندرا موری',
                email: 'leeWheeler@infotech.io',
                img: '/static/img/avatars/thumb-13.jpg',
            },
            {
                id: '5',
                name: 'یوجین استوارت',
                email: 'joyce991@infotech.io',
                img: '/static/img/avatars/thumb-5.jpg',
            },
        ],
        labels: ['اولویت پایین'],
        attachments: [],
        comments: [
            {
                id: 'bAvBdtKHNC',
                name: 'استیو ساتون',
                src: '/static/img/avatars/thumb-11.jpg',
                message:
                    'هلفتیکا ۸-بیت فوتوبوث تامبلر کامیون غذا. پین مینای دندان گرگ شلوغ سارتوریال، برانچ شوریدچ اسکیت‌بورد ریش هلفتیکا. تایپ‌نویس گاستروپاب سفارشی.',
                date: new Date(y, 4, 20),
            },
        ],
        dueDate: new Date(y, 7, 5),
        checked: false,
    },
]

const processingCards = [
    {
        id: 'ywejrdr3rn',
        name: 'اصلاح چیدمان داشبورد',
        description:
            'بایودیزل سلوِیج لترپرس ۸-بیت، کتاب رنگ‌آمیزی بانجو آستین پابست پست-آیرونیک تلاش‌سخت بدون گلوتن تیله منشور اصیل موی بافته.',
        cover: '',
        members: [
            {
                id: '2',
                name: 'جرمایا مینسک',
                email: '',
                img: '/static/img/avatars/thumb-2.jpg',
            },
            {
                id: '8',
                name: 'جسیکا ولز',
                email: 'iamfred@imaze.infotech.io',
                img: '/static/img/avatars/thumb-8.jpg',
            },
        ],
        labels: ['اشکال'],
        attachments: [],
        comments: [
            {
                id: 'afPugkEmkp',
                name: 'Angelina Gotelli',
                src: '/static/img/avatars/thumb-1.jpg',
                message: '',
                date: new Date(y, 5, 16),
            },
        ],
        dueDate: new Date(y, 3, 17),
        checked: false,
    },
    {
        id: 'tkBXWJGwkr',
        name: 'طراحی جدید',
        description:
            'تایپ‌نویس جهنم نان ابری سلامت گاث ریش میلک‌شیک فور لوکو.',
        cover: '',
        members: [
            {
                id: '10',
                name: 'ارل مایلز',
                email: 'brittany1134@gmail.com',
                img: '/static/img/avatars/thumb-10.jpg',
            },
        ],
        labels: ['وظیفه'],
        attachments: [
            {
                id: 'NjHJhHeWrG',
                name: 'issue.jpg',
                src: '/static/img/others/img-16.jpg',
                size: '46.1kb',
            },
        ],
        comments: [
            {
                id: 'MAsPDzGwnA',
                name: 'مکس الکساندر',
                src: '/static/img/avatars/thumb-3.jpg',
                message:
                    'اف تراست فاند مگینگز، مدیتیشن چیچارونز برانچ ولف نروال شوریدچ هرچه حقوق دوچرخه رمپ‌ها.',
                date: new Date(y, 4, 20),
            },
        ],
        dueDate: null,
        checked: false,
    },
    {
        id: 'VQgUDrYJYH',
        name: 'بهبود تجربه کاربری',
        description:
            'جاوا، باریستا، تک‌منشأ، فوم کهنه، شناسه قطره‌چکان ارگانیک در زمین‌ها شناسه ترکی. زمین‌های پرس فرانسوی وینی، قهوه تیره قوی که قاشق اضافی قهوه با شیر کافه او لایت کاپوچینو. دوپیو، قهوه، آفوگاتو، لاته لاغر اتحادیه، کاراجیلو سیاه کرما فوری تیره.',
        cover: '',
        members: [
            {
                id: '6',
                name: 'آرلین پیرس',
                email: 'samanthaphil@infotech.io',
                img: '/static/img/avatars/thumb-6.jpg',
            },
        ],
        labels: ['اولویت پایین', 'وظیفه'],
        attachments: [],
        comments: [],
        dueDate: new Date(y, 4, 20),
        checked: false,
    },
]

const submittedCards = [
    {
        id: 'jzjn95g3v4',
        name: 'به‌روزرسانی محیط نود',
        description:
            'یونیکورن اشغال لوکاوور پاگ، استامپ‌تاون به معنای واقعی کلمه توفو آیرونی. اف هنر خیابانی پالئو شوریدچ. بان می قبل از اینکه آنها زغال فعال را بفروشند.',
        cover: '',
        members: [
            {
                id: '3',
                name: 'مکس الکساندر',
                email: 'ronnie_vergas@infotech.io',
                img: '/static/img/avatars/thumb-3.jpg',
            },
        ],
        labels: ['اولویت پایین'],
        attachments: [],
        comments: [
            {
                id: 'nBAGhJqe9v',
                name: 'مکس الکساندر',
                src: '/static/img/avatars/thumb-3.jpg',
                message: '',
                date: new Date(y, 1, 8),
            },
        ],
        dueDate: null,
        checked: false,
    },
    {
        id: 'ZFQDPmscwA',
        name: 'حذف داده‌های کاربر',
        description:
            'صلیب پولاروید مرغ داغ نامتقارن گرگ هلوتیکا کیتار مد تبر رمپ‌های یوولو ویفرهای 90s.',
        cover: '/static/img/others/img-15.jpg',
        members: [
            {
                id: '9',
                name: 'کامیلا سیمونز',
                email: 'carolyn_h@gmail.com',
                img: '/static/img/avatars/thumb-9.jpg',
            },
            {
                id: '10',
                name: 'ارل مایلز',
                email: 'brittany1134@gmail.com',
                img: '/static/img/avatars/thumb-10.jpg',
            },
        ],
        labels: ['مسئله زنده'],
        attachments: [
            {
                id: 'NjHJhHeWrG',
                name: 'issue.jpg',
                src: '/static/img/others/img-15.jpg',
                size: '42.6kb',
            },
        ],
        comments: [
            {
                id: 'nBAGhJqe9v',
                name: 'مکس الکساندر',
                src: '/static/img/avatars/thumb-3.jpg',
                message:
                    'لورم خالد ایپسوم یک کلید اصلی برای موفقیت است. کلید این است که هر کلیدی را داشته باشید، کلید باز کردن هر در. من در حال انجام کاری هستم. زندگی چیزی است که شما از آن می‌سازید، پس بیایید آن را بسازیم.',
                date: new Date(y, 1, 8),
            },
        ],
        dueDate: null,
        checked: false,
    },
]

const completedCards = [
    {
        id: 'yhjk5679xr',
        name: 'آماده برای تست',
        description:
            'آب سبز سلامت گوت، ویلیامزبرگ نان تست چهار دلاری شبی شیک مگینگز شلوار جین شورت وی‌اچ‌اس.',
        cover: '',
        members: [
            {
                id: '13',
                name: 'کاساندرا موری',
                email: 'leeWheeler@infotech.io',
                img: '/static/img/avatars/thumb-13.jpg',
            },
            {
                id: '9',
                name: 'کامیلا سیمونز',
                email: 'carolyn_h@gmail.com',
                img: '/static/img/avatars/thumb-9.jpg',
            },
        ],
        labels: ['وظیفه'],
        attachments: [],
        comments: [
            {
                id: 'yxc5gwrXUZ',
                name: 'مکس الکساندر',
                src: '/static/img/avatars/thumb-3.jpg',
                message:
                    'نوآوری واقعی اغلب از استارتاپ کوچک می‌آید که به اندازه کافی لاغر است تا یک بازار را راه‌اندازی کند اما فاقد قدرت برای مالکیت آن است.',
                date: new Date(y, 3, 4),
            },
        ],
        dueDate: new Date(y, 3, 4),
        checked: true,
    },
    {
        id: 'UMgvapYVXm',
        name: 'اتصال API کند',
        description:
            'آب سبز سلامت گوت، ویلیامزبرگ نان تست چهار دلاری شبی شیک مگینگز شلوار جین شورت وی‌اچ‌اس.',
        cover: '',
        members: [
            {
                id: '5',
                name: 'یوجین استوارت',
                email: 'joyce991@infotech.io',
                img: '/static/img/avatars/thumb-5.jpg',
            },
            {
                id: '6',
                name: 'آرلین پیرس',
                email: 'samanthaphil@infotech.io',
                img: '/static/img/avatars/thumb-6.jpg',
            },
            {
                id: '7',
                name: 'روبرتا هورتون',
                email: 'taratarara@imaze.edu.du',
                img: '/static/img/avatars/thumb-7.jpg',
            },
        ],
        labels: ['باگ'],
        attachments: [],
        comments: [],
        dueDate: new Date(y, 7, 19),
        checked: true,
    },
    {
        id: 'uRZNVsCmDW',
        name: 'ورود ناموفق',
        description:
            'گیاه هوا کاشی مترو چهار لوکو رمپ‌ها. میکرو دوزینگ افال توت بگ، قهوه تک‌منشأ بیودیزل قبل از اینکه آنها بفروشند.',
        cover: '',
        members: [
            {
                id: '4',
                name: 'شانون بیکر',
                email: 'cookie_lukie@hotmail.com',
                img: '/static/img/avatars/thumb-4.jpg',
            },
        ],
        labels: ['مسئله زنده'],
        attachments: [],
        comments: [],
        dueDate: new Date(y, 4, 6),
        checked: true,
    },
    {
        id: 'PBSGmhVgvS',
        name: 'محلی نادرست',
        description:
            'هودی سبیل بیدار پور-اور شما احتمالاً از آنها نشنیده‌اید کرای.',
        cover: '',
        members: [
            {
                id: '5',
                name: 'یوجین استوارت',
                email: 'joyce991@infotech.io',
                img: '/static/img/avatars/thumb-5.jpg',
            },
            {
                id: '3',
                name: 'مکس الکساندر',
                email: 'ronnie_vergas@infotech.io',
                img: '/static/img/avatars/thumb-3.jpg',
            },
        ],
        labels: ['اولویت پایین'],
        attachments: [],
        comments: [
            {
                id: 'dNskbPFeQD',
                name: 'مکس الکساندر',
                src: '/static/img/avatars/thumb-3.jpg',
                message:
                    'اف تراست فاند مگینگز، مدیتیشن چیچارونز برانچ ولف نروال شوریدچ هرچه حقوق دوچرخه رمپ‌ها.',
                date: new Date(y, 4, 20),
            },
            {
                id: 'qNzkmRZiTO',
                name: 'جرمایا مینسک',
                src: '/static/img/avatars/thumb-2.jpg',
                message: 'توجه شد!',
                date: new Date(y, 4, 20),
            },
        ],
        dueDate: new Date(y, 7, 13),
        checked: true,
    },
]

export const issueData = {
    ticketId: '#PD-127',
    title: 'API کار نمی کند',
    createdBy: 'Angelina Gotelli',
    underProject: 'Sprint 2',
    status: 'در حال انجام',
    priority: 'بالا',
    description: `<p>از چارچوب‌های چابک برای ارائه خلاصه‌ای قوی برای مرورهای سطح بالا استفاده کنید. رویکردهای تکراری برای استراتژی شرکت، تفکر مشارکتی را برای پیشبرد ارزش پیشنهادی کلی تقویت می کند. از طریق تنوع در محل کار و توانمندسازی، دیدگاه جهانی کل نگر از نوآوری مخرب را به طور ارگانیک رشد دهید.</p>
  <p>هومن را به بردگی می‌برم که به شخص بی‌خبر یا گربه‌های تاکسیدو که همیشه ظاهری نازک دارند، روی علف‌ها مدفوع می‌کنند یا در خواب روی سگ‌ها غرغر می‌کنند، اما عشق چشمک می‌زند و خرخر خمیازه خمیازه می‌کشد.</p>
  <p>تصمیم بگیرید که امروز هیچ کاری به صاحب من نداشته باشید و فرشتان را در جاهایی که همه می‌توانند ببینند ببندید - چرا مهارت‌های هنری خارق‌العاده‌ام در پنجه زدن را پنهان کنم؟</p>
  <p> بینی خز انسان خود را روی لباس صاحبش گاز بگیرید، پس شکمم را بخراشید، در واقع از شما متنفرم، حالا با من مبارزه کنید تا مبل را در 3 ثانیه منفجر کنم.</p>
	`,
    dueDate: 1742795479,
    assignees: [
        {
            id: '2',
            name: 'Jeremiah Minsk',
            email: 'terrance_moreno@infotech.io',
            img: '/static/img/avatars/thumb-2.jpg',
        },
        {
            id: '3',
            name: 'Max Alexander',
            email: 'ronnie_vergas@infotech.io',
            img: '/static/img/avatars/thumb-3.jpg',
        },
    ],
    labels: [
        {
            id: '1',
            title: 'اشکال',
        },
        {
            id: '2',
            title: 'موضوع زنده',
        },
    ],
    comments: [
        {
            id: 'Wx8FDSsVTg',
            name: 'Arlene Pierce',
            src: '/static/img/avatars/thumb-6.jpg',
            message:
                'کامیون مواد غذایی تامبلر غرفه 8 بیتی هلوتیکا. مینای سنجاق گرگ ژولیده sartorial، برانچ shoreditch اسکیت بورد ریش helvetica. ماشین تحریر شطرنجی gastropub سفارشی.',
            date: new Date(y, 4, 20),
        },
        {
            id: '3AhkqqSFFr',
            name: 'Roberta Horton',
            src: '/static/img/avatars/thumb-7.jpg',
            message:
                '<strong>@Angelina</strong> یکی از دلایل اصلی سقوط امپراتوری روم این بود که -عدم صفر- آنها راهی برای نشان دادن پایان موفقیت آمیز برنامه های C خود نداشتند. ',
            date: new Date(y, 4, 20),
        },
    ],
    attachments: [
        {
            id: 'jubuK7XGp3',
            name: 'mail.jpg',
            src: '/static/img/others/img-13.jpg',
            size: '36.1kb',
        },
        {
            id: 'NjHJhHeWrG',
            name: 'issue.jpg',
            src: '/static/img/others/img-16.jpg',
            size: '46.1kb',
        },
    ],
    activity: [
        {
            type: 'UPDATE-TICKET',
            dateTime: 1646580000,
            ticket: 'PD-127',
            status: 1,
            userName: 'Angelina Gotelli',
            userImg: '',
        },
        {
            type: 'COMMENT',
            dateTime: 1646578417,
            userName: 'Arlene Pierce',
            userImg: '/static/img/avatars/thumb-1.jpg',
            comment: `کامیون مواد غذایی تامبلر غرفه 8 بیتی هلوتیکا. مینای سنجاق گرگ ژولیده sartorial، برانچ shoreditch اسکیت بورد ریش helvetica. ماشین تحریر شطرنجی gastropub سفارشی.`,
        },
        {
            type: 'ADD-TAGS-TO-TICKET',
            dateTime: 1646574027,
            userName: 'Eugene Stewart',
            tags: ['موضوع زنده', 'اشکال'],
        },
        {
            type: 'ADD-FILES-TO-TICKET',
            dateTime: 1646569123,
            userName: 'Shannon Baker',
            files: ['document.csv'],
            ticket: 'PD-1092',
        },
        {
            type: 'COMMENT-MENTION',
            dateTime: 1646565473,
            userName: 'Roberta Horton',
            userImg: '/static/img/avatars/thumb-7.jpg',
            comment: `<strong>@Angelina</strong> یکی از دلایل اصلی سقوط امپراتوری روم این بود که -عدم صفر- آنها راهی برای نشان دادن پایان موفقیت آمیز برنامه های C خود نداشتند.`,
        },
        {
            type: 'ASSIGN-TICKET',
            dateTime: 1646554397,
            userName: 'Lee Wheeler',
            assignee: 'Alvin Moreno',
            ticket: 'PD-1092',
        },
    ],
}

export const scrumboardData = {
    'To Do': toBeProcessCards,
    'In Progress': processingCards,
    'To Review': submittedCards,
    Completed: completedCards,
}

export const taskBugFix = [
    {
        id: '0a2ff03d-1b61-4ab0-aa43-e5c7f4578a79',
        name: 'قادر به بارگذاری فایل نیست',
        dueDate: '2024-08-05T00:00:00.000Z',
        checked: false,
        progress: 'در حال انجام',
        priority: 'بالا',
        assignee: {
            name: 'جسیکا ولز',
            img: '/static/img/avatars/thumb-8.jpg',
        },
    },
    {
        id: 'da1d38c9-e7ad-4d7d-88d4-bc88e152d10e',
        name: 'خطا در پرس و جو پایگاه داده',
        dueDate: '2024-07-15T00:00:00.000Z',
        checked: true,
        progress: 'تکمیل شده',
        priority: 'متوسط',
        assignee: {
            name: 'جرمایاه مینسک',
            img: '/static/img/avatars/thumb-2.jpg',
        },
    },
    {
        id: 'cd820d94-aa38-40f0-97ab-2a5c82f3c701',
        name: 'مشکل احراز هویت',
        dueDate: '2024-09-20T00:00:00.000Z',
        checked: false,
        progress: 'در حال انجام',
        priority: 'بالا',
        assignee: {
            name: 'ارل مایلز',
            img: '/static/img/avatars/thumb-10.jpg',
        },
    },
    {
        id: 'f5bfcff3-975c-4b22-a49b-1eeb8a3c03ec',
        name: 'خطا در قابلیت جستجو',
        dueDate: '2024-09-05T00:00:00.000Z',
        checked: false,
        progress: 'در حال انجام',
        priority: 'بالا',
        assignee: {
            name: 'مکس الکساندر',
            img: '/static/img/avatars/thumb-3.jpg',
        },
    },
    {
        id: 'c773847d-7f60-4d27-b3b0-6f61915e9b1a',
        name: 'مشکل سازگاری با فایرفاکس',
        dueDate: '2024-07-25T00:00:00.000Z',
        checked: true,
        progress: 'تکمیل شده',
        priority: 'متوسط',
        assignee: {
            name: 'Angelina Gotelli',
            img: '/static/img/avatars/thumb-1.jpg',
        },
    },
]

export const taskDevelopment = [
    {
        id: '9ff33d5e-2f1c-4b20-b8ae-2241ed9cc624',
        name: 'بهینه سازی عملکرد',
        dueDate: '2024-08-30T00:00:00.000Z',
        checked: false,
        progress: 'در انتظار',
        priority: 'متوسط',
        assignee: {
            name: 'Jeremiah Minsk',
            img: '/static/img/avatars/thumb-2.jpg',
        },
    },
    {
        id: 'a6951cbb-fb0d-4223-b73a-8b8b9e40f0d2',
        name: 'ادغام درگاه پرداخت',
        dueDate: '2024-10-15T00:00:00.000Z',
        checked: false,
        progress: 'در انتظار',
        priority: 'پایین',
        assignee: {
            name: 'Jeremiah Minsk',
            img: '/static/img/avatars/thumb-2.jpg',
        },
    },
    {
        id: 'b671d721-4d5e-4b63-8827-739e8d5cb22c',
        name: 'طرح بندی صفحه نمایه کاربر را به روز کنید',
        dueDate: '2024-08-10T00:00:00.000Z',
        checked: false,
        progress: 'در حال انجام',
        priority: 'بالا',
        assignee: {
            name: 'Max Alexander',
            img: '/static/img/avatars/thumb-3.jpg',
        },
    },
    {
        id: 'f4d29527-84e6-4702-92d6-805b6a703dc8',
        name: 'اقدامات امنیتی را افزایش دهید',
        dueDate: '2024-08-20T00:00:00.000Z',
        checked: false,
        progress: 'در انتظار',
        priority: 'متوسط',
        assignee: {
            name: 'Arlene Pierce',
            img: '/static/img/avatars/thumb-6.jpg',
        },
    },
]

export const taskUiUx = [
    {
        id: 'b8d49ba2-ae0e-4567-aa82-ef057f0a2d2b',
        name: 'تنظیم UI برای داشبورد',
        dueDate: '2024-09-25T00:00:00.000Z',
        checked: false,
        progress: 'در حال انجام',
        priority: 'بالا',
        assignee: {
            name: 'Jeremiah Minsk',
            img: '/static/img/avatars/thumb-2.jpg',
        },
    },
    {
        id: 'fffb790d-fc52-4df6-8403-07128cc6fb31',
        name: 'بهبود UX برای فرآیند ورود',
        dueDate: '2024-08-15T00:00:00.000Z',
        checked: false,
        progress: 'در انتظار',
        priority: 'متوسط',
        assignee: {
            name: 'Arlene Pierce',
            img: '/static/img/avatars/thumb-6.jpg',
        },
    },
    {
        id: 'b32d5d2b-f762-426f-9bbd-ec15c879e1a5',
        name: 'یک ظاهر طراحی عنصر UI برای صفحه محصول',
        dueDate: '2024-10-05T00:00:00.000Z',
        checked: false,
        progress: 'در انتظار',
        priority: 'پایین',
        assignee: {
            name: 'Angelina Gotelli',
            img: '/static/img/avatars/thumb-1.jpg',
        },
    },
]

export const taskPlaning = [
    {
        id: '8e00e8d5-b87e-4c97-8d80-695ff91f50b0',
        name: 'برنامه ریزی نقشه راه پروژه استراتژیک',
        dueDate: '2024-09-30T00:00:00.000Z',
        checked: false,
        progress: 'در حال انجام',
        priority: 'بالا',
        assignee: {
            name: 'Jeremiah Minsk',
            img: '/static/img/avatars/thumb-2.jpg',
        },
    },
    {
        id: 'a1a1d440-8f4c-4be4-a92b-249107fd4e1d',
        name: 'طرح تخصیص منابع فصلی',
        dueDate: '2024-08-20T00:00:00.000Z',
        checked: false,
        progress: 'در انتظار',
        priority: 'متوسط',
        assignee: {
            name: 'Arlene Pierce',
            img: '/static/img/avatars/thumb-6.jpg',
        },
    },
    {
        id: 'c18b5a49-43f1-4dd3-bf5a-3f4e5e4b3db2',
        name: 'جلسه برنامه ریزی استراتژیک کسب و کار',
        dueDate: '2024-10-10T00:00:00.000Z',
        checked: false,
        progress: 'در انتظار',
        priority: 'پایین',
        assignee: {
            name: 'Angelina Gotelli',
            img: '/static/img/avatars/thumb-1.jpg',
        },
    },
]

export const tasksData = {
    'رفع اشکال': taskBugFix,
    'توسعه': taskDevelopment,
    'طراح رابط کاربری': taskUiUx,
    'برنامه ریزی': taskPlaning,
}
