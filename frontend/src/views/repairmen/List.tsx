...
const Repairmen = () => {
    ...
    const fetchAdmins = async () => {
        try {
            setLoading(true)
            const response = await axiosInstance.get('/api/admins/list?type=1')
            setAdmins(response.data.results)
            setError(null)
        } catch (error: any) {
            message.error('خطا در دریافت لیست تعمیرکاران')
            setError('خطا در دریافت لیست تعمیرکاران')
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleDelete = async (id: number) => {
        Modal.confirm({
            title: 'حذف تعمیرکار',
            content: 'آیا از حذف این تعمیرکار اطمینان دارید؟',
            okText: 'بله',
            cancelText: 'خیر',
            onOk: async () => {
                try {
                    await axiosInstance.delete(`/api/admins/detail/${id}`)
                    message.success('تعمیرکار با موفقیت حذف شد')
                    fetchAdmins()
                } catch (error: any) {
                    message.error('خطا در حذف تعمیرکار')
                }
            },
        })
    }
